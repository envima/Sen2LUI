...
#'
#' Per Default, the ffsp starts with all possible 2-pair combinations.
#' minVar allows to start the selection with more than 2 variables, e.g.
#' minVar=3 starts the ffsp testing all combinations of 3 (instead of 2) variables
...
#' \dontrun{
#' data(iris)
#' ffsmodel <- ffsp(iris[, 1:4], iris$Species)
#' ffsmodel$selectedvars
#' ffsmodel$selectedvars_perf
#' }
#'
#' # or perform model with target-oriented validation (LLO CV)
#' # the example is described in Gasch et al. (2015). The ffsp approach for this dataset is described in
...
#' predictors <- c(
#'   "DEM", "TWI", "BLD", "Precip_cum", "cday", "MaxT_wrcc",
#'   "Precip_wrcc", "NDRE.M", "Bt", "MinT_wrcc", "Northing", "Easting"
#' )
#'
#' # run ffsp model with Leave Location out CV
#' set.seed(10)
#' ffsmodel <- ffsp(trainDat[, predictors], trainDat$VW,
#'   method = "rf",
#'   tuneLength = 1, trControl = ctrl
#' )
#' ffsmodel
#'
#' # compare to model without ffsp:
...
ffsp <- function(predictors,
  ...
  acc <- 0
  perf_all <- data.frame(matrix(
    ncol = length(predictors) + 3,
  ...
  perf_all_par <- foreach(i = 1:nrow(minGrid), .combine = rbind, .errorhandling = "stop", .packages = c("caret")) %d% {
    ...
    if (verbose) {
      print(paste0(
        "maximum number of models that still need to be trained: ",
  ...
  for (acc in seq(nrow(perf_all_par))) {
    perf_all[acc, 1:perf_all_par[acc, "length.variablenames."]] <- perf_all_par[acc, 1:2]
    perf_all[acc, (length(predictors) + 1):ncol(perf_all)] <- perf_all_par[acc, c("actmodelperf", "actmodelperfSE", "length.variablenames.")]
  }
  ...
  if (verbose) {
    print(paste0(
      paste0("vars selected: ", paste(selectedvars, collapse = ",")),
  ...
  for (k in 1:(length(names(predictors)) - minVar)) {
    ...
    if (length(startvars) < (k + (minVar - 1))) {
      message(paste0(
        "Note: No increase in performance found using more than ",
    ...
    for (i in 1:length(nextvars)) {
      ...
      perf_all[acc, (length(predictors) + 1):ncol(
        perf_all
      )] <- c(actmodelperf, actmodelperfSE, length(variablenames))
      if (verbose) {
        print(paste0(
          "maximum number of models that still need to be trained: ",
          round(choose(n, minVar) + (n - minVar) * (n - minVar + 1) / 2 - acc, 0)
